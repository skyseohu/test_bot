...
temp_TOKEN= 'N/z/k/4/N/D/I/0/N/T/k/5/N/D/k/4/M/D/Y/z/O/D/k/y/./X/_/0/0/2/g/./W/g/r/p/8/M/r/_/g/f/p/J/r/h/B/b/d/4/U/Q/H/A/U/k/V/V/o/'
TOKEN = ''
for i in range(len(temp_TOKEN)): #디스코드 자동 토큰 짤림 없애주는 코드
    if i%2==0:
        TOKEN += temp_TOKEN[i]
...
async def on_message(message):  #봇이 쓰는 명령에는 반응 안되게 하는 부분
    ...
    if message.content.startswith("!디렉토리확인"):
        await message.channel.send(os.getcwd())
        await message.channel.send(os.path.dirname(os.path.realpath(__file__)))
        await message.channel.send(__file__)
        await message.channel.send(os.path.realpath(__file__))
        await message.channel.send(os.path.abspath(__file__))
    ...
    if message.content.startswith("!강화"):
        ...
        os.chdir("D:\\3. 코딩\\2. 디스코드 봇\\discord-bot-white201-main\\discord bot 1.1\\discord bot 1.1")
